...
Remove-Item -Force -ErrorAction Ignore ./artifacts/linux-binaries-arnakwallet-v$version.tar.gz
Remove-Item -Force -ErrorAction Ignore ./artifacts/linux-deb-arnakwallet-v$version.deb
Remove-Item -Force -ErrorAction Ignore ./artifacts/Windows-binaries-arnakwallet-v$version.zip
Remove-Item -Force -ErrorAction Ignore ./artifacts/Windows-installer-arnakwallet-v$version.msi
Remove-Item -Force -ErrorAction Ignore ./artifacts/macOS-arnakwallet-v$version.dmg
...
bash src/scripts/mkmacdmg.sh --qt_path ~/Qt/5.11.1/clang_64/ --version $version --zcash_path ~/prod/arnak 
if (! $?) {
    Write-Output "[Error]"
    exit 1;
}
...
if (! (Test-Path ./artifacts/linux-binaries-arnakwallet-v$version.tar.gz) -or
    ! (Test-Path ./artifacts/linux-deb-arnakwallet-v$version.deb) -or
    ! (Test-Path ./artifacts/Windows-binaries-arnakwallet-v$version.zip) -or
    ! (Test-Path ./artifacts/macOS-arnakwallet-v$version.dmg) -or 
    ! (Test-Path ./artifacts/Windows-installer-arnakwallet-v$version.msi) ) {
        Write-Host "[Error]"
        exit 1;
    }
Write-Host "[OK]"
