            ...
            if (reply->error() == QNetworkReply::NoError) {
                ...
                if (maxVersion > currentVersion && (!silent || maxVersion > maxHiddenVersion)) {
                    ...
                    if (ans == QMessageBox::Yes) {
                        QDesktopServices::openUrl(QUrl("https://github.com/michailduzhanski/awallet/releases"));
                    } else {
...
void RPC::shutdownZcashd() {
    ...
    QObject::connect(&waiter, &QTimer::timeout, [&] () {
        ...
        if ((ezcashd->atEnd() && ezcashd->processId() == 0) ||
            waitCount > 30 || 
            conn->config->zcashDaemon)  {   // If arnakd is daemon, then we don't have to do anything else
            qDebug() << "Ended";
            waiter.stop();
